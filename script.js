// Array containing paragraphs for the typing game
const paragraphs = [
...
function loadParagraph() {
    // Selecting a random paragraph from the array
    const ranIndex = Math.floor(Math.random() * paragraphs.length);

    // Clearing the typing text content
    typingText.innerHTML = "";

    // Splitting the paragraph into characters and creating spans for each
    paragraphs[ranIndex].split("").forEach(char => {
        let span = `<span>${char}</span>`;
        typingText.innerHTML += span;
    });

    // Adding "active" class to the first character
    typingText.querySelectorAll("span")[0].classList.add("active");

    // Setting up event listeners for focusing on input field
...
function initTyping() {
    // Getting characters and typed character
    let characters = typingText.querySelectorAll("span");
    let typedChar = inpField.value.split("")[charIndex];

    // Checking if there are characters left and time is remaining
    if (charIndex < characters.length - 1 && timeLeft > 0) {
        // Handling the start of typing
        if (!isTyping) {
            timer = setInterval(initTimer, 1000);
            isTyping = true;
        }

        // Handling typed characters and updating styles
        if (typedChar == null) {
            if (charIndex > 0) {
                charIndex--;
        ...
        // Updating active character
        characters.forEach(span => span.classList.remove("active"));
        characters[charIndex].classList.add("active");

        // Calculating and updating WPM, mistakes, and CPM
        ...
        // Game over, clearing timer and input field
        clearInterval(timer);
        inpField.value = "";
    }
}
...
function initTimer() {
    if (timeLeft > 0) {
        // Updating time left and WPM
        timeLeft--;
        timeTag.innerText = timeLeft;
        let wpm = Math.round(((charIndex - mistakes) / 5) / (maxTime - timeLeft) * 60);
        wpmTag.innerText = wpm;
    } else {
        // Game over, clearing timer
        clearInterval(timer);
    }
}
...
function resetGame() {
    // Loading a new paragraph and resetting game variables
    loadParagraph();
    clearInterval(timer);
    timeLeft = maxTime;
...
// Initial loading of a paragraph
loadParagraph();

// Event listeners for input and try-again button
